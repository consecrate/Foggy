...
def _render_wrapped_review(
    ...
    progress_markup = _build_wrapped_progress(progress, pill_count)
    ...
    # Wrapped cards always use Anki's native reviewer bottom bar.

    return f"""
<style>
:root {{
  --foggy-bg: oklch(0.20 0.01 240);
  --foggy-bg-muted: oklch(0.24 0.01 240);
  --foggy-bg-panel: oklch(0.28 0.01 240);
  --foggy-bg-panel-strong: oklch(0.22 0.01 240);
  --foggy-surface: oklch(0.32 0.01 240);
  --foggy-text: oklch(0.98 0 0);
  --foggy-text-muted: oklch(0.82 0.01 240);
  --foggy-text-subtle: oklch(0.65 0.01 240);
  --foggy-text-dim: oklch(0.50 0.01 240);
  --foggy-accent: oklch(0.79 0.19 130);
  --foggy-accent-hover: oklch(0.85 0.19 130);
  --foggy-accent-dark: oklch(0.62 0.19 130);
  --foggy-danger: oklch(0.60 0.20 25);
  --foggy-warning: oklch(0.80 0.16 85);
  --foggy-good: oklch(0.68 0.15 250);
  --foggy-white-wash-6: oklch(0.98 0 0 / 0.06);
...
.foggy-wrap-progress-track {{
  --foggy-wrap-progress-gap: 8px;
  --foggy-wrap-progress-target-width: 460px;
  --foggy-wrap-progress-target-count: 10;
  --foggy-wrap-progress-pill-count: 10;
  --foggy-wrap-progress-pill-width: calc(
    (
        var(--foggy-wrap-progress-target-width) -
          (
            (var(--foggy-wrap-progress-target-count) - 1) *
              var(--foggy-wrap-progress-gap)
          )
      ) / var(--foggy-wrap-progress-target-count)
  );
  display: grid;
  grid-template-columns: repeat(10, minmax(0, 1fr));
  gap: var(--foggy-wrap-progress-gap);
  width: min(
    100%,
    calc(
      (
          var(--foggy-wrap-progress-pill-count) *
            var(--foggy-wrap-progress-pill-width)
        ) +
        (
          (var(--foggy-wrap-progress-pill-count) - 1) *
            var(--foggy-wrap-progress-gap)
        )
    )
  );
...
.foggy-wrap-progress-segment.is-goal {{
  box-shadow: 0 0 0 1px oklch(0.79 0.19 130 / 0.4);
}}
...
.foggy-wrap-button--primary {{
  ...
  font-size: 15px;
  border-radius: 16px;
  box-shadow: 0 4px 0 var(--foggy-accent-dark);
  transition:
...
.foggy-wrap-button--primary:hover {{
  ...
  box-shadow: 0 6px 0 var(--foggy-accent-dark);
}}

.foggy-wrap-button--primary:active {{
  transform: translateY(4px);
  box-shadow: 0 0 0 var(--foggy-accent-dark);
}}

.foggy-wrap-button--primary:disabled {{
  color: var(--foggy-text-dim);
  background: var(--foggy-white-wash-6);
  box-shadow: none;
  cursor: not-allowed;
  transform: none;
}}
...
.foggy-wrap-reward-panel {{
  width: min(100%, 760px);
  padding: 22px;
  border: 1px solid oklch(0.35 0.01 240 / 0.42);
  border-radius: 28px;
  background:
    linear-gradient(180deg, oklch(0.28 0.01 240 / 0.98), oklch(0.22 0.01 240 / 0.98));
  box-shadow: 0 28px 64px oklch(0.1 0.01 240 / 0.28);
}}
  ...
  continueButton.addEventListener("click", function() {{
    ...
    if (progress) {{
      progress.setAttribute(
        "aria-label",
        "Review streak: 0 of {pill_count} cards"
      );
    }}
...
def _get_wrapped_review_state(card, kind: str) -> tuple[int, bool, int, int]:
    if kind == "reviewQuestion":
        available_card_count = _review_queue_card_count(card)
        progress, show_reward, reward_key = _wrapped_review_state.state_for_question(
            card.id,
            available_card_count=available_card_count,
        )
        return (progress, show_reward, reward_key, _wrapped_review_state.pill_count)

    progress, _, reward_key = _wrapped_review_state.current_state()
    return (progress, False, reward_key, _wrapped_review_state.pill_count)


def _build_wrapped_progress(progress: int, pill_count: int) -> str:
    segments = []
    for index in range(pill_count):
        classes = ["foggy-wrap-progress-segment"]
        if index < progress:
            classes.append("is-filled")
        if index == pill_count - 1:
    ...
    return (
        ...
        f'aria-label="{_wrapped_progress_label(progress, pill_count)}">'
        '<div class="foggy-wrap-progress-track" '
        f'style="--foggy-wrap-progress-pill-count: {pill_count}; '
        f'grid-template-columns: repeat({pill_count}, minmax(0, 1fr));">'
        + "".join(segments)
        + "</div>"
        "</div>"
    )
...
<div id="foggy-wrap-reward" class="foggy-wrap-reward">
  <div class="foggy-wrap-reward-panel">
    ...
    <p class="foggy-wrap-reward-copy">Ten cards down. Take the cat, then jump back in.</p>
...
def _maybe_prefetch_wrapped_reward(progress: int, show_reward: bool) -> None:
    if not _wrapped_review_state.rewards_enabled:
        return
...
def _build_wrapped_review_payload(
    progress: int,
    show_reward: bool,
    reward_key: int,
    pill_count: int,
    ...
    return {
        ...
        "rewardImageUrl": _reward_image_src(reward_key) if show_reward else "",
    }


def _review_queue_card_count(card) -> int | None:
    sched = getattr(getattr(mw, "col", None), "sched", None)
    if sched is None:
        return None

    counts_method = getattr(sched, "counts", None)
    counts = None
    ...
    if counts is None:
        fallback_counts = [getattr(sched, attr, None) for attr in ("newCount", "lrnCount", "reviewCount")]
        counts = fallback_counts if any(count is not None for count in fallback_counts) else None

    normalized = _normalize_queue_counts(counts)
    return sum(normalized) if normalized is not None else None


def _normalize_queue_counts(counts) -> tuple[int, int, int] | None:
    if not isinstance(counts, (list, tuple)):
        return None

    raw_counts = counts
    if len(counts) == 2 and isinstance(counts[1], (list, tuple)):
        raw_counts = counts[1]
...
def _wrapped_progress_label(progress: int, pill_count: int) -> str:
    return f"Review streak: {progress} of {pill_count} cards"
